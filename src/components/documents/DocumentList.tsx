...
const STATUS_STYLES: Record<DocumentListItem['status'], { label: string; cls: string }> = {
  processing: { label: 'Elaborazione…', cls: 'bg-neon-blue/10 text-white border border-neon-blue/30 animate-pulse' },
    ...
    <div className="space-y-2">
      {documents.map((doc) => {
        ...
        return (
          <div
            key={doc.id}
            className="glass border-2 border-white/10 hover:border-neon-blue/50 transition-colors duration-300 hover-lift flex items-center gap-4 px-4 py-3"
          >
